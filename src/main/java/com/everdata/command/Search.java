...
public class Search {
	...
	public ArrayList<AST_Stats.Bucket> bucketFields = null;
	...
	*/
	...
	public static void LeafTraverse(final ArrayList<Function> statsFields, final ArrayList<AST_Stats.Bucket> bucketFields, Queue<List<String>> rows,
			MultiBucketsAggregation.Bucket node, Stack<String> path, int[] totalRows,
		...
		if( bucketAgg != null ){
			Iterator<? extends MultiBucketsAggregation.Bucket> iterator = bucketAgg.getBuckets().iterator();
			while (iterator.hasNext()) {
	...
	public static void buildReport(int from, int size, XContentBuilder builder,
		...
		// 构建表头
		for(AST_Stats.Bucket b : response.bucketFields){
		...
		if (report.get("statsWithBy") != null) {
			...
			Iterator<? extends MultiBucketsAggregation.Bucket> iterator = bucketAgg.getBuckets().iterator();
			while (iterator.hasNext()) {
				MultiBucketsAggregation.Bucket next = iterator.next();
	...
	public SearchResponse executeQueryWithNonJoin(int from, final int size, String[] sortFields) {
		...
		return querySearch.execute().actionGet();
	}

	public QueryResponse executeQuery( int from, final int size, String[] sortFields) {
	...
	public SearchResponse executeReport(int from, int size) {
		reportSearch.setSearchType(SearchType.COUNT);
		dumpSearchScript(reportSearch, logger);
		return reportSearch.execute().actionGet();
	}
